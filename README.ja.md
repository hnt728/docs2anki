...
`docs2anki` は、PDFの指定ページをGeminiで一問一答カード化し、Anki向けにエクスポートできるツールです。

## 主な機能

- PDFをアップロードして、指定ページ範囲からカードを生成
- `ranges` / `step` / `overlap` でチャンク分割を調整
- 実行前にページとチャンク境界をプレビュー
- 進捗、警告、失敗チャンクをUI上で確認
- 生成後のカードをブラウザ上で編集
...
- UI文言は現在日本語です。
- PDFプレビューはCDN経由の `pdf.js` を使うため、プレビュー表示にはネットワーク接続が必要です。
